...
4. **Derive a kebab-case filename:** `YYYY-MM-DD-{title-slug}.md`

5. **Write recap note** directly to filesystem (backtick safety):
   ```
...
6. **Confirm:** `Session saved → Memory/sessions/YYYY-MM-DD-{title-slug}.md`
...
- **Tree-graph linking.** `related:` has `[[sessions]]` as first entry (folder parent), then up to 2 discovered dependency notes. Max 3 entries total. No sibling links to other sessions.
